...
impl Outgoing {
    ...
    fn buffer_to_messages(&self) -> Vec<String> {
        ...
        let possible_messages = buffer_string.split("Content-Length:");
        for possible_message in possible_messages.filter(|msg| !msg.is_empty()) {
            let lines: Vec<&str> = possible_message.split("\r\n").collect();
            if lines.len() < 3 {
...
{"method": "initialize", "params": { "capabilities": {}}, "jsonrpc": "2.0", "id": 1}"#.replace("\n", "\r\n");
    ...
    fn test_buffer_to_messages_partial_message_one_line() {
        ...
        assert_eq!(outgoing.buffer.lock().unwrap().len(), message.len());
    }
...
{"method": "initialize", "params": { "capabilities": {}}, "jso"#.replace("\n", "\r\n");
        let buffer: Vec<u8> = message.as_bytes().into();
...
{"method": "initialize", "params": { "capabilities": {}}, "jsonrpc": "2.0", "id": 1}Content-Le"#.replace("\n", "\r\n");
        ...
        assert_eq!(outgoing.buffer.lock().unwrap().len(), "Content-Le".len());
    }
...
{"method": "initialize", "params": { "capabilities": {}}, "jsonrpc": "2.0", "id": 1}Content-Length: 1"#.replace("\n", "\r\n");
        ...
        assert_eq!(messages.len(), 1);
        assert_eq!(outgoing.buffer.lock().unwrap().len(), "Content-Length: 1".len());
    }
...
}}, "jsonrpc": "2.0", "id": 1}Content-Length: 1"#.replace("\n", "\r\n");
        let buffer: Vec<u8> = message.as_bytes().into();
